...
// When BOTH the nightly `allocator_api` feature AND the `allocator-api2` crate feature
// are enabled simultaneously (which happens when doublets and regalloc2/SpacetimeDB are
// combined in the same project), we need a separate impl of allocator_api2::Allocator
// alongside the std::alloc::Allocator impl. Without this, bumpalo only implements the
// nightly std trait (due to the cfg condition on line ~31-32), causing regalloc2 to fail
// because it needs the allocator_api2 trait to be satisfied.
#[cfg(all(feature = "allocator_api", feature = "allocator-api2"))]
unsafe impl<'a, const MIN_ALIGN: usize> allocator_api2::alloc::Allocator for &'a Bump<MIN_ALIGN> {
    ...
    unsafe fn grow_zeroed(
        ...
        let mut ptr = <Self as allocator_api2::alloc::Allocator>::grow(self, ptr, old_layout, new_layout)?;
        ptr.as_mut()[old_layout.size()..].fill(0);
        Ok(ptr)
    }
}
