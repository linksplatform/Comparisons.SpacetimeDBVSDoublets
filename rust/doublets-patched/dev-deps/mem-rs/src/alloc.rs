use crate::{RawMem, Result, base::Base, internal};
use std::{
    ...
    unsafe fn alloc_impl(&mut self, capacity: usize) -> Result<&mut [T]> {
        ...
        let result: Result<_> = try {
            if self.base.ptr.as_non_null_ptr() == NonNull::dangling() {
                let layout = Layout::array::<T>(capacity).map_err(crate::traits::Error::from)?;
                self.alloc
                    .allocate(layout)
                    .map_err(crate::traits::Error::from)?
            } else {
                let old_layout =
                    Layout::array::<T>(old_capacity).map_err(crate::traits::Error::from)?;
                let new_layout =
                    Layout::array::<T>(new_capacity).map_err(crate::traits::Error::from)?;
...
impl<T, A: Allocator> Drop for Alloc<T, A> {
    fn drop(&mut self) {
        ...
        let _: Result<_> = try {
            ...
            let layout = Layout::array::<T>(ptr.len()).map_err(crate::traits::Error::from)?;
            // SAFETY: ptr is valid slice
            unsafe {
