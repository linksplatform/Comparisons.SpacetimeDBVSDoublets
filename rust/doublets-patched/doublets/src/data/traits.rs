...
impl<T: LinkType, All: Doublets<T> + Sized> DoubletsExt<T> for All {
    ...
    type ImplIter = impl Iterator<Item = Link<T>>;

    #[inline]
    fn iter(&self) -> Self::ImplIter {
        let cap = self
            .count_by([self.constants().any; 3].to_query())
            .as_usize();
        let mut vec = Vec::with_capacity(cap);
        self.each_by([self.constants().any; 3], &mut |link| {
            vec.push(link);
            Flow::Continue
        });
        vec.into_iter()
    ...
    #[cfg(feature = "small-search")]
    type ImplIterSmall = impl Iterator<Item = Link<T>>;
    ...
    fn iter_small(&self) -> Self::ImplIterSmall {
        ...
        let mut vec = smallvec::SmallVec::<[Link<_>; SIZE_HINT]>::with_capacity(
            self.count_by([self.constants().any; 3].to_query())
                .as_usize(),
        );
        self.each_by([self.constants().any; 3], |link| {
            vec.push(link);
            Flow::Continue
        });
        vec.into_iter()
